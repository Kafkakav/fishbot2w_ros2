...
void wificonfig_byuart(void)
{
    ...
    while (count < 128)
    {
        ...
        else if(c >= 0xC2) { //UTF-8
            int ic;
            char utf8buf[8]={0};

            if (c >= 0xC2 && c <= 0xDF) {  // 2 bytes
                ic = uart_read_bytes(UART_NUM_0, (uint8_t *)&utf8buf[0], 1, 20 / portTICK_PERIOD_MS);
                if (ic > 0 && (utf8buf[0] & 0xC0) == 0x80) {
                    line[count++] = c;
                    line[count++] = utf8buf[0];
                }
            } else if (c >= 0xE0 && c <= 0xEF) {  // 3 bytes
                ic = uart_read_bytes(UART_NUM_0, (uint8_t *)&utf8buf[0], 2, 20 / portTICK_PERIOD_MS);
                if (ic == 2 && (utf8buf[0] & 0xC0) == 0x80 && (utf8buf[1] & 0xC0) == 0x80) {
                    line[count++] = c;
                    line[count++] = utf8buf[0];
                    line[count++] = utf8buf[1];
                }
            } else if (c >= 0xF0 && c <= 0xF4) {  // 4 bytes
                ic = uart_read_bytes(UART_NUM_0, (uint8_t *)&utf8buf[0], 3, 20 / portTICK_PERIOD_MS);
                if (ic == 3 && (utf8buf[0] & 0xC0) == 0x80 && (utf8buf[1] & 0xC0) == 0x80 && (utf8buf[2] & 0xC0) == 0x80) {
                    line[count++] = c;
                    line[count++] = utf8buf[0];
                    line[count++] = utf8buf[1];
                    line[count++] = utf8buf[2];
                }
            }
        }
        vTaskDelay(10 / portTICK_PERIOD_MS);

...
int uart_read_byte(uint8_t *cbuf, int waitms) {
    while (waitms > 0) {
        if( Serial.available() > 0 ) {
            *cbuf = (uint8_t)Serial.read();
            return 1;
        }
        delay(10);
        waitms -= 10;
...
int8_t FishBotConfig::loop_config_uart(int c, char result[][32])
{
    ...
    else if (c > 0 && c < 127)
    {
        ...
        if (c >= 0xC2 && c <= 0xDF) {  // 2 bytes
            ic = uart_read_byte((uint8_t *)&utf8buf[0], 50);
            if (ic > 0 && (utf8buf[0] & 0xC0) == 0x80) {
                line[index++] = c;
                line[index++] = utf8buf[0];
            }
            line[index] = 0;
        } else if (c >= 0xE0 && c <= 0xEF) {  // 3 bytes
            for(n=0,ic=0; n<2; n++) 
               ic += uart_read_byte((uint8_t *)&utf8buf[n], 50);
            if (ic == 2 && (utf8buf[0] & 0xC0) == 0x80 && (utf8buf[1] & 0xC0) == 0x80) {
                line[index++] = c;
                line[index++] = utf8buf[0];
                line[index++] = utf8buf[1];
            }
            line[index] = 0;
